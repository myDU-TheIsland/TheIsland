...
namespace TheIsland.Framework.Services
{
    ...
    public class StoreService : IStoreService
    {
        ...
        public StoreService(IDualPlayerRepository dualPlayerRepository, IStorePurchaseHistoryRepository storePurchaseHistoryRepository, IStoreItemRepository storeItemRepository, DualUniverseSettings settings, IGeneralBot bot)
        {
        ...
        public async Task<string> SaveImage(IFormFile image)
        {
            ...
            if (!Directory.Exists(this._settings.StoreImagePath))
            {
                Directory.CreateDirectory(this._settings.StoreImagePath);
            }
        ...
        public async Task<StorePurchaseHistory> PurchaseItem(double playerId, string discordId, double itemId, double quantity)
        {
            ...
            try
            {
                ...
                IEnumerable<StorePurchaseHistory> historyOfPurchase = await this._storePurchaseHistoryRepository.GetPlayerPurchaseHistory(playerId).ConfigureAwait(false);
                ...
                {
                    foreach (KeyValuePair<double, double> purchaseItem in item.content.Items)
                    {
                        ...
                        IGameplayDefinition? gamebankItem = this._gameplayBank.GetDefinition(Convert.ToUInt64(purchaseItem.Key));
                        ...
                        ItemAndQuantity inventoryItem = new ItemAndQuantity
                        {
                ...
                {
                    foreach (KeyValuePair<double, string> purchaseSkin in item.content.Skins)
                    {
        ...
        internal async Task<List<string>> GiveTalentPointInjector(double playerId, double amount)
        {
            ...
            {
                double multi = this.GetTalentPointPercentage(totalPoints + addPoints);
                double addition = step * multi;
        ...
        internal async Task<bool> DebitPlayerWallet(double playerId, double amount, string reason)
        {
            ...
            EntityId sourceWallet = new EntityId { playerId = Convert.ToUInt64(playerId) };
            await this._sql
