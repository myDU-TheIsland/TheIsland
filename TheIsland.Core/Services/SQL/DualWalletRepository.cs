...
TIME_SERIES AS (
  ...
  FROM GENERATE_SERIES
        ( (SELECT MIN(date_trunc('hour', time)) FROM public.wallet_operation)
        , (SELECT MAX(date_trunc('hour', time)) FROM public.wallet_operation)
        , '1 hour'::interval)),
MARKET_TRANS AS (
SELECT 
...
LEFT JOIN MARKET_TRANS ON
	TIME_SERIES.hour = date_trunc('hour', MARKET_TRANS.time) 
